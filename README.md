...
- **계산 기능**
  ...
  - 계산기에서는 수식과 계산결과를 따로 보여주고 있으며 수식은 사용자가 입력한 커맨드를 바탕으로 배열에 저장하고 표기는 `join()`메서드를 활용해서 문자열로 이어서 보여주도록 하고 있습니다
  - **실시간으로 바뀌는 계산결과**는 사용자가 숫자나 연산자를 눌렀을 때 그 전에 입력한 값을 토대로 계산해서 보여주도록 하고 있습니다
